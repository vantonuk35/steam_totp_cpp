...
inline std::string SHA1::final(bool hex)
{
    ...
    else
    {
        for (size_t i = 0; i < sizeof(digest) / sizeof(digest[0]); i++)
        {
            result << (char)((digest[i] & 0xFF000000) >> 24);
            result << (char)((digest[i] & 0xFF0000) >> 16);
            result << (char)((digest[i] & 0xFF00) >> 8);
